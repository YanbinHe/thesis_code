% clc
% clear
% load 'testGraphQ32.mat'
% solIdx = 1;
%% test 31 dimension constant update
% input description:
...
if solIdx == 1
    ...
    L = 2*Lini;
elseif solIdx == 3
    % filled with code generated Wds and Lbar
    proj_q = eye(num_nodes) - 1/(q'*q)*(q*q');
    [Upen,observe,~] = svd(proj_q);% rank n-1
    U_proj = Upen(:,1:end-1);
    U_proji = U_proj;
...
% ./Eaext(end-numC + 1:end)./Eaext(end-numC + 1:end)
if solIdx == 1
...
elseif solIdx == 3
[Projext,~] = svd(U_projext*Veffext(:,end-numC + 1:end));
Projext = Projext(:,1:numC)*Projext(:,1:numC)';

[Basis1,~] = svd(U_projnew*GenEigV(:,end-numC + 1:end));
Basis1Proj = Basis1(:,1:numC)*Basis1(:,1:numC)';

[Basis2,~] = svd(U_proji*GenEigVi(:,end-numC + 1:end));
Basis2Proj = Basis2(:,1:numC)*Basis2(:,1:numC)';

dif(2,simu) = 1 - 1/3*(abs(((U_projext*Veffext(:,end + 1 -3))'*U_projnew*GenEigV(:,end + 1 -3))/(norm(U_projext*Veffext(:,end + 1 -3))*norm(U_projnew*GenEigV(:,end + 1 -3)))) + abs(((U_projext*Veffext(:,end + 1 -2))'*U_projnew*GenEigV(:,end + 1 -2))/(norm(U_projext*Veffext(:,end + 1 -2))*norm(U_projnew*GenEigV(:,end + 1 -2))))+ abs(((U_projext*Veffext(:,end + 1 -1))'*U_projnew*GenEigV(:,end + 1 -1))/(norm(U_projext*Veffext(:,end + 1 -1))*norm(U_projnew*GenEigV(:,end + 1 -1)))));
dif(3,simu) = 1 - 1/3*(abs(((U_projext*Veffext(:,end + 1 -3))'*U_proji*GenEigVi(:,end + 1 -3))/(norm(U_projext*Veffext(:,end + 1 -3))*norm(U_proji*GenEigVi(:,end + 1 -3))))+abs(((U_projext*Veffext(:,end + 1 -2))'*U_proji*GenEigVi(:,end + 1 -2))/(norm(U_projext*Veffext(:,end + 1 -2))*norm(U_proji*GenEigVi(:,end + 1 -2))))+abs(((U_projext*Veffext(:,end + 1 -1))'*U_proji*GenEigVi(:,end + 1 -1))/(norm(U_projext*Veffext(:,end + 1 -1))*norm(U_proji*GenEigVi(:,end + 1 -1)))));
dif(5,simu) = norm(-Projext/trace(Projext) + Basis1Proj/trace(Basis1Proj),'fro');
dif(6,simu) = norm(-Projext/trace(Projext) + Basis2Proj/trace(Basis2Proj),'fro');
...
% =======
% %     ---
% >>>>>>> 01be4b6a05128df26bc989f8193982c71be63b9a
%     tic;
%     b = diag(Db);
%     u = diag(Du);
